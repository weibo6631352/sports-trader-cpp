...
public:
    ...
    SecureBuffer(SecureBuffer&& other) noexcept : buf_(other.buf_) { sodium_memzero(other.buf_.data(), N); }
    ...
    // 析构: 清零 (防止私钥留在内存)
    ~SecureBuffer() noexcept { sodium_memzero(buf_.data(), N); }
...
public:
    ...
    [[nodiscard]] static std::array<std::uint8_t, kEd25519SignatureBytes> sign(
        const SecureBuffer<kEd25519SecretKeyBytes>& secret_key,
    ...
    [[nodiscard]] static bool generate_keypair(std::array<std::uint8_t, kEd25519PublicKeyBytes>& pk_out,
