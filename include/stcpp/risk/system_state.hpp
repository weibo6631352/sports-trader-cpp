...
[[nodiscard]] constexpr std::string_view to_string(SystemState s) noexcept {
    switch (s) {
        ...
        case SystemState::DRAIN:    return "DRAIN";
        case SystemState::HALTED:   return "HALTED";
        case SystemState::DECAYED:  return "DECAYED";
    }
    return "unknown";
}
...
class StateMachine {
 public:
    explicit StateMachine(SystemState init = SystemState::RUNNING) noexcept
        : state_(init) {}
    ...
    [[nodiscard]] bool try_transition(SystemState from, SystemState to) noexcept {
        // DECAYED 是吸收态, 不可再迁移
        if (from == SystemState::DECAYED) return false;
    ...
    [[nodiscard]] SystemState get() const noexcept {
 ...
 private:
    ...
    static_assert(sizeof(std::atomic<SystemState>) <= 4,
                  "SystemState CAS must be lock-free on target arch");
};

// lock-free static assert (运行期不适用, 编译期 hint)
static_assert(std::atomic<SystemState>::is_always_lock_free ||
              sizeof(SystemState) == 1,
