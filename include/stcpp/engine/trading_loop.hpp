...
struct UserFill;
struct OnchainPosition;  // live 启动对账 (完整定义 include/stcpp/polymarket/onchain_positions.hpp)
}  // namespace stcpp::polymarket
...
public:
    ...
    std::size_t SeedLivePositionsFromOnchain(const std::vector<polymarket::OnchainPosition>& positions);
