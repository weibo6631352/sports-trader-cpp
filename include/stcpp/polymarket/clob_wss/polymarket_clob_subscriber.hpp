...
public:
    ...
    // 只读订阅计数 getter (观测侧, R-12 const, 不改热路径行为)
    // hot_token_count / cold_token_count: vCPU0 写端集合大小 (读端仅做计数, 无锁安全)
    // user_condition_count: user channel 订阅的 condition_id 数量
    // 注: hot_token_ids_ / cold_token_ids_ / user_condition_ids_ 仅 vCPU0 写;
    //     观测线程调用此 getter 为快照读取, 接受轻微 ABA (size_t 读原子, R-12 合规).
    [[nodiscard]] std::size_t hot_token_count() const noexcept { return hot_token_ids_.size(); }
    [[nodiscard]] std::size_t cold_token_count() const noexcept { return cold_token_ids_.size(); }
    [[nodiscard]] std::size_t user_condition_count() const noexcept { return user_condition_ids_.size(); }
