...
[[nodiscard]] constexpr std::string_view SubTopicName(SubTopic t) noexcept {
    switch (t) {
        case SubTopic::kMarket:
            return "market";
        case SubTopic::kGame:
            return "game";
        case SubTopic::kOutcomes:
            return "outcomes";
        case SubTopic::kBook:
            return "book";
        case SubTopic::kPriceChange:
            return "price_change";
        case SubTopic::kLastTradePrice:
            return "last_trade_price";
        case SubTopic::kTickSizeChange:
            return "tick_size_change";
        case SubTopic::kSystemStatus:
            return "system";
    }
    return "";
}
...
struct FourTs {
    ...
    [[nodiscard]] constexpr bool IsMonotonic() const noexcept {
        return event_ts_ns <= data_source_ts_ns && data_source_ts_ns <= ingestion_ts_ns &&
...
struct PriceLevelDelta {
    ...
    std::uint64_t size_micro = 0;
    std::uint8_t side = 0;
    std::int64_t level_ts_ns = 0;
};
...
struct SystemStatusUpdate {
    FourTs ts{};
    SystemHealth health = SystemHealth::kHealthy;
};
...
struct WssEvent {
    ...
    [[nodiscard]] const FourTs& ts() const noexcept {
        switch (topic) {
            case SubTopic::kGame:
                return payload.game.ts;
            case SubTopic::kLastTradePrice:
                return payload.trade.ts;
            case SubTopic::kTickSizeChange:
                return payload.tick.ts;
            case SubTopic::kOutcomes:
                return payload.outcomes.ts;
            case SubTopic::kSystemStatus:
                return payload.system.ts;
            default:
                return payload.book.ts;
        }
    }
};
