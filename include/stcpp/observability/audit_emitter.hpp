...
public:
    ...
    [[nodiscard]] ResultT emit_unlock(const RiskDecisionInput& ctx, std::string_view operator_id) noexcept;
    [[nodiscard]] ResultT emit_strategy_decayed(const RiskDecisionInput& ctx) noexcept;
    [[nodiscard]] ResultT emit_recon_drift(const RiskDecisionInput& ctx, std::string_view diag) noexcept;
    ...
    [[nodiscard]] bool hash_chain_verify(std::uint64_t start_seq, std::uint64_t end_seq) const noexcept;
    ...
    [[nodiscard]] ResultT emit_with_injected_chain(const RiskDecisionInput& in, AuditEventType type_override,
                                                   const Hash256& prev_hash, const Hash256& payload_hash,
                                                   const Hash256& current_hash, std::uint64_t seq) noexcept;
...
public:
    ...
    AuditEmitterPool(WriterT* risk_writer, WriterT* signer_writer, WriterT* ml_writer, WriterT* stats_writer,
                     WriterT* strategy_writer) noexcept;

    // pool 主 emit (代理到对应 origin 的 emitter, 保证全局 chain 串联)
    [[nodiscard]] ResultT emit(AuditOrigin origin, const RiskDecisionInput& in) noexcept;
    [[nodiscard]] ResultT emit_safe_mode_enter(AuditOrigin origin, const RiskDecisionInput& ctx,
                                               std::string_view reason) noexcept;
    [[nodiscard]] ResultT emit_safe_mode_exit(AuditOrigin origin, const RiskDecisionInput& ctx) noexcept;
    [[nodiscard]] ResultT emit_state_transition(AuditOrigin origin, const RiskDecisionInput& ctx,
                                                std::string_view from_to) noexcept;
    [[nodiscard]] ResultT emit_unlock(AuditOrigin origin, const RiskDecisionInput& ctx,
                                      std::string_view operator_id) noexcept;
    [[nodiscard]] ResultT emit_strategy_decayed(AuditOrigin origin, const RiskDecisionInput& ctx) noexcept;
    [[nodiscard]] ResultT emit_recon_drift(AuditOrigin origin, const RiskDecisionInput& ctx,
...
private:
    ...
    [[nodiscard]] ChainStep alloc_chain_step_locked(const RiskDecisionInput& in, AuditOrigin origin) noexcept;
};
