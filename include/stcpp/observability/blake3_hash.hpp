...
class Blake3Hasher {
 public:
    ...
    [[nodiscard]] static Hash256 hash_chain(
        const Hash256& prev_hash,
        const Hash256& payload_hash) noexcept {
#if defined(BLAKE3_REAL)
    ...
    [[nodiscard]] static Hash256 compute_payload_hash(
        std::uint64_t seq,
        std::uint8_t  event_type_byte,
 ...
 private:
#if defined(BLAKE3_REAL)
    [[nodiscard]] static Hash256 hash_256_real(
    ...
    [[nodiscard]] static Hash256 hash_chain_real(
        const Hash256& prev,
        const Hash256& payload) noexcept {
    ...
    [[nodiscard]] static Hash256 hash_256_stub(
        const std::uint8_t* data, std::size_t len) noexcept {
        Hash256 out{};
        if (len > 0) {
    ...
    [[nodiscard]] static Hash256 hash_chain_stub(
        const Hash256& prev,
        const Hash256& payload) noexcept {
        Hash256 out{};
