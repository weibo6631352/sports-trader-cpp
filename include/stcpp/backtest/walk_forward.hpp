...
public:
    ...
    [[nodiscard]] std::vector<WalkForwardWindow> generate(std::int64_t data_start_ns,
...
public:
    ...
    [[nodiscard]] bool is_trade_accessible(TradeRecord const& t) const noexcept {
        return t.as_of_ts_ns >= window_start_ns_ && t.as_of_ts_ns <= window_end_ns_ && trade_ts_ok(t);
    }

    // 过滤出本 guard 窗口内的 trade (含 PIT 校验)
    [[nodiscard]] std::vector<TradeRecord> filter_trades(std::vector<TradeRecord> const& trades) const {
    ...
    template <Phase Q = P>
    std::enable_if_t<Q == Phase::InSample, void> freeze_params(ParamSet const& params) {
        frozen_params_ = params;
        params_frozen_ = true;
    }

    // 读取已冻结的参数 (OOS/Frozen 阶段用)
    [[nodiscard]] ParamSet const& get_frozen_params() const noexcept { return frozen_params_; }

    [[nodiscard]] bool has_frozen_params() const noexcept { return params_frozen_; }
...
public:
    ...
    [[nodiscard]] std::vector<FoldSplit> split(std::vector<TradeRecord> const& trades) const {
        ...
        for (std::uint32_t k = 0; k < n_folds_; ++k) {
            ...
            for (auto const& t : trades) {
                ...
                if (it != game_to_fold.end() && it->second == k) {
                    ...
                    if (t.as_of_ts_ns > val_max_ts)
                        val_max_ts = t.as_of_ts_ns;
                    if (t.as_of_ts_ns < val_min_ts)
                        val_min_ts = t.as_of_ts_ns;
                }
            }
