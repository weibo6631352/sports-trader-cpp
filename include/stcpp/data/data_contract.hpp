...
inline constexpr std::array<std::string_view, 8> kBookmakerNames{
    "10Bet", "WilliamHill", "bet365", "Marathon", "Unibet", "BetVictor", "1xBet", "Betano",
};
...
[[nodiscard]] inline PayloadValidationResult validate_payload(
    std::int64_t event_ts_ns, std::int64_t data_source_ts_ns, std::int64_t ingestion_ts_ns,
    std::int64_t as_of_ts_ns, DataSourceTsOrigin ds_origin, PayloadType pt,
    const DataContract& = kDefaultDataContract) noexcept {
    (void)pt;
    PayloadValidationResult r{};
    if (event_ts_ns <= 0) {
        r.error_msg = "event_ts_ns must be > 0";
        return r;
    }
    if (data_source_ts_ns <= 0) {
        r.error_msg = "data_source_ts_ns must be > 0 (UPSTREAM_PAYLOAD)";
        return r;
    }
    if (ingestion_ts_ns <= 0) {
        r.error_msg = "ingestion_ts_ns must be > 0";
        return r;
    }
    if (as_of_ts_ns <= 0) {
        r.error_msg = "as_of_ts_ns must be > 0";
        return r;
    }
    if (data_source_ts_ns < event_ts_ns) {
        r.error_msg = "data_source_ts < event_ts (PIT)";
        return r;
    }
    if (ingestion_ts_ns < data_source_ts_ns) {
        r.error_msg = "ingestion_ts < data_source_ts (PIT)";
        return r;
    }
    if (as_of_ts_ns < ingestion_ts_ns) {
        r.error_msg = "as_of_ts < ingestion_ts (PIT)";
        return r;
    }
