...
struct FeatureSnapshotRecord {
    ...
    // ml::kFeatureCount features (float32, NaN = missing, ADR-008: [4]=Goalserve_devig_p_yes_fair)
    //   R-fee-2: 33 (= 32 + fee_rate_coef[32]); 跟随 ml::kFeatureCount, 禁再硬编 32.
    float feat[ml::kFeatureCount]{};
    ...
    static FeatureSnapshotRecord from_snapshots(const ml::FeatureSnapshot& fs, const ml::TrainingLabel& tl,
        ...
        for (std::size_t i = 0; i < ml::kFeatureCount; ++i)
