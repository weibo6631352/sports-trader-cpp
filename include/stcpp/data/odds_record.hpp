...
struct MultiBookOddsRecord {
    ...
    [[nodiscard]] std::size_t serialize_into(std::span<std::byte> out) const noexcept {
        if (out.size() < max_serialized_size()) return 0;
        std::size_t off = 0;
        ...
        wi64(ts.event_ts_ns); wi64(ts.data_source_ts_ns);
        wi64(ts.ingestion_ts_ns); wi64(ts.as_of_ts_ns);
        wu8(static_cast<std::uint8_t>(ts.ds_origin));
        wu8(static_cast<std::uint8_t>(sport));
        wu8(0); wu8(0);  // pad

        for (const auto& sl : slots) {
            wf64(sl.odds_yes); wf64(sl.odds_no);
            wu8(sl.valid ? 1u : 0u);
            for (int p = 0; p < 7; ++p) wu8(0u);
        }
        wu32(crc32c);
    ...
    [[nodiscard]] std::size_t ValidBookmakerCount() const noexcept {
        std::size_t n = 0;
        for (const auto& sl : slots)
            if (sl.valid && sl.odds_yes > 1.0 && sl.odds_no > 1.0) ++n;
        return n;
    }
};
