...
BacktestMetrics compute_full_metrics(std::vector<TradeRecord> const& trades, WalkForwardWindow const& window,
                                     TradeBucket bucket, bool is_in_sample, double bankroll,
                                     std::size_t n_bootstrap, std::size_t block_size,
                                     std::uint64_t bootstrap_seed, std::size_t n_bonferroni_experiments,
    ...
    for (auto const& t : trades) {
        if (!t.filtered_out && t.bucket == bucket && t.outcome != SettleOutcome::Pending) {
    ...
    std::sort(sorted.begin(), sorted.end(),
              [](TradeRecord const& a, TradeRecord const& b) { return a.as_of_ts_ns < b.as_of_ts_ns; });
    ...
    auto const daily_rets = MetricsComputer::compute_daily_returns(sorted, bankroll, win_start, win_end);
