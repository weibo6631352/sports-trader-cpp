...
BuildResult TraderDaemon::Build() {
    ...
    // 2026-06-15 数据驱动收紧 0.65→0.70 (老板「调试可关程序」授权下全量回填 481 结算做的定论分析):
    //   sharp moneyline 厚盘子集跨 4 次独立回放 (n=32→481) 一致显示 fair≥0.70 是 EV 峰值 (94%胜 +0.171/股
    //   vs 0.65 的 82%胜 +0.127/股; 0.75 反崩 77%胜 −0.058)。量代价极小 (只掉 [0.65,0.70) 那 ~1 个输家盘,
    //   不掉赢家)。沿老板「赢面≥X 越高越稳」方向再进一步。可逆 (CLV 监控异常即回 0.65)。
    cfg_.trading_loop.min_open_fair = 0.70;
