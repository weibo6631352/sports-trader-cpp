...
// 私有辅助: 时钟分数 [0, 1]
// ---------------------------------------------------------------------------
double BaselineFairValueModel::time_fraction_(
    ...
    if (!std::isfinite(frac)) return 0.0;
...
std::optional<double> BaselineFairValueModel::extract_microprice_(
    ...
    if (book_row == nullptr) return std::nullopt;
    // 只接受 YES side (订单簿 microprice 是 YES token 的成交概率估计)
    if (book_row->token_side != "YES") return std::nullopt;
    double const mp = book_row->microprice;
    // NaN/Inf 防护
    if (!std::isfinite(mp)) return std::nullopt;
    // 有效范围: (kProbEps, kProbMax) — 极端值视为无效
    if (mp <= kProbEps || mp >= kProbMax) return std::nullopt;
    return mp;
}
...
// ---------------------------------------------------------------------------
FairValueResult BaselineFairValueModel::estimate(
    ...
    int const score_diff_i =
        static_cast<int>(game_row.score_home_total) -
        static_cast<int>(game_row.score_away_total);
    ...
    double const log_odds_prior =
        params_.alpha * score_diff + params_.beta * time_frac;
    ...
    if (microprice.has_value()) {
        ...
        // 线性混合 (加权平均)
        p_yes_adj = kappa_used * microprice.value() +
                    (1.0 - kappa_used) * p_yes_prior;
        // 防御性检查: 混合结果仍需 finite
