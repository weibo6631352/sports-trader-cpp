...
std::string parse_val(std::string_view arg, std::string_view prefix) {
    return (arg.substr(0, prefix.size()) == prefix) ? std::string(arg.substr(prefix.size())) : std::string{};
}

bool parse_args(int argc, char** argv, CliArgs& out) {
    for (int i = 1; i < argc; ++i) {
        std::string_view a{argv[i]};
        if (a == "--emergency-override") {
            out.emergency_override = true;
            continue;
        }
        ...
        if (try_parse("--strategy-id=", out.strategy_id))
            continue;
        if (try_parse("--trigger-audit-id=", out.trigger_audit_id_hex))
            continue;
        if (try_parse("--laohan-sig=", out.laohan_sig_b64))
            continue;
        if (try_parse("--laotang-sig=", out.laotang_sig_b64))
            continue;
        if (try_parse("--laolei-sig=", out.laolei_sig_b64))
            continue;
        if (try_parse("--exec-mode=", out.exec_mode))
            continue;
...
bool validate(CliArgs const& a) {
    if (a.strategy_id.empty()) {
        std::cerr << "[ERROR] --strategy-id required\n";
        return false;
    }
    if (a.trigger_audit_id_hex.size() != 32) {
        std::cerr << "[ERROR] --trigger-audit-id must be 32 hex chars\n";
        return false;
    }
    if (a.exec_mode != "paper" && a.exec_mode != "live") {
        std::cerr << "[ERROR] --exec-mode=paper|live\n";
        return false;
    }
    if (!a.emergency_override) {
        if (a.laohan_sig_b64.empty()) {
            std::cerr << "[ERROR] --laohan-sig required\n";
            return false;
        }
        if (a.laotang_sig_b64.empty()) {
            std::cerr << "[ERROR] --laotang-sig required\n";
            return false;
        }
    }
    if (a.laolei_sig_b64.empty()) {
        std::cerr << "[ERROR] --laolei-sig required\n";
        return false;
    }
    return true;
}

bool load_pubkey(const char* env_name, std::array<std::uint8_t, stcpp::cli::kEd25519PubKeyBytes>& out) {
    const char* val = std::getenv(env_name);
    if (!val || !val[0]) {
        std::cerr << "[ERROR] env " << env_name << " not set\n";
        return false;
    }
...
std::array<std::uint8_t, 16> make_audit_id(std::int64_t ts_ns, std::uint8_t seq) noexcept {
    ...
    id[8] = seq;
    id[9] = static_cast<std::uint8_t>(ts_ns & 0xFF);
    return id;
}

// W6 stub IPC (W7 接 unix domain socket RPC)
void emit_ipc(std::string_view mode, std::array<std::uint8_t, 16> const& a1,
              std::array<std::uint8_t, 16> const& a2, std::array<std::uint8_t, 16> const& a3,
              bool emergency) {
    std::cout << "[IPC-STUB] {\"cmd\":\"set_state\",\"state\":\"RUNNING\",\"exec_mode\":\"" << mode
              << "\",\"emergency\":" << (emergency ? "true" : "false") << ",\"audit_ids\":[\"" << hex16(a1)
              << "\",\"" << hex16(a2) << "\",\"" << hex16(a3) << "\"]}\n"
...
int main(int argc, char** argv) {
    ...
    if (sodium_init() < 0) {
        std::cerr << "[FATAL] sodium_init() failed\n";
        return 1;
    }

    CliArgs args;
    if (!parse_args(argc, argv, args) || !validate(args))
        return 1;

    stcpp::cli::SignerInput laohan_in{}, laotang_in{}, laolei_in{};
    if (!args.emergency_override) {
        if (!load_pubkey("RM_UNLOCK_PUBKEY_LAOHAN", laohan_in.pubkey))
            return 1;
        if (!load_pubkey("RM_UNLOCK_PUBKEY_LAOTANG", laotang_in.pubkey))
            return 1;
    }
    if (!load_pubkey("RM_UNLOCK_PUBKEY_LAOLEI", laolei_in.pubkey))
        return 1;
    ...
    if (ts_env && ts_env[0]) {
        ...
        // R-20 §2.2 UTC epoch_ns 豁免: CLI 签名时间戳, 无上游 ts source
        auto tp = std::chrono::system_clock::now();  // UTC epoch_ns: local ingestion_ts
        now_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(tp.time_since_epoch()).count();
    }

    std::string payload = verifier.build_payload(args.strategy_id, args.trigger_audit_id_hex, now_ns);
