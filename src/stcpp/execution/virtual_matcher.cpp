...
VirtualFill VirtualMatcher::Match(const VirtualOrder& order) noexcept {
    ...
    {
        const std::size_t mlen = std::min(order.market_id.size(), static_cast<std::size_t>(32));
...
VirtualFill VirtualMatcher::MatchWithBook(const VirtualOrderWithBook& order) noexcept {
    ...
    const auto clob_out =
        microstructure::FillRateModel::compute_from_clob_book(order.book, order.probe, intent_with_size);
    ...
    if (out.fill_price <= 0.0)
        out.fill_price = 0.001;
