...
GameState ParseGameStateStr(std::string_view body) noexcept {
    if (body.find("\"game_state\":\"live\"")      != std::string_view::npos) return GameState::kLive;
    if (body.find("\"game_state\":\"pregame\"")   != std::string_view::npos) return GameState::kPregame;
    if (body.find("\"game_state\":\"halftime\"")  != std::string_view::npos) return GameState::kHalftime;
    if (body.find("\"game_state\":\"ended\"")     != std::string_view::npos) return GameState::kEnded;
    if (body.find("\"game_state\":\"cancelled\"") != std::string_view::npos) return GameState::kCancelled;
    if (body.find("\"game_state\":\"postponed\"") != std::string_view::npos) return GameState::kPostponed;
...
PMWssSubscriber::~PMWssSubscriber() { Stop(); }
...
bool FillBaseTs(std::string_view body, std::int64_t recv_ts_ns, FourTs& ts,
    ...
    std::int64_t ts_ms = 0;
    if (!ExtractInt64(body, "timestamp", ts_ms)) return false;
...
bool PMWssSubscriber::ParseMarket(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    if (!FillBaseTs(body, recv_ts_ns, u.ts)) return false;
    ...
    std::int64_t nr = 0;
    if (ExtractInt64(body, "neg_risk", nr)) u.neg_risk = static_cast<std::uint8_t>(nr ? 1 : 0);
    u.is_snapshot = 1;
...
bool PMWssSubscriber::ParseGame(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    if (!FillBaseTs(body, recv_ts_ns, g.ts)) return false;
    ...
    if (ExtractInt64(body, "period", period))       g.period           = static_cast<std::uint16_t>(period);
    if (ExtractInt64(body, "time_remaining_s", tr)) g.time_remaining_s = static_cast<std::int32_t>(tr);
    if (ExtractInt64(body, "score_home", sh))       g.score_home       = static_cast<std::int32_t>(sh);
    if (ExtractInt64(body, "score_away", sa))       g.score_away       = static_cast<std::int32_t>(sa);
    if (ExtractInt64(body, "sport", sport))         g.sport            = static_cast<std::uint16_t>(sport);
...
bool PMWssSubscriber::ParseOutcomes(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    if (!FillBaseTs(body, recv_ts_ns, o.ts, DataSourceTsOrigin::kInferredFromDsTs)) return false;
    ExtractHexId(body, "condition_id", o.market_id);
    if      (body.find("\"resolution_status\":\"resolved\"")  != std::string_view::npos) o.resolution_status = ResolutionStatus::kResolved;
    else if (body.find("\"resolution_status\":\"resolving\"") != std::string_view::npos) o.resolution_status = ResolutionStatus::kResolving;
    else                                                                                 o.resolution_status = ResolutionStatus::kOpen;
...
bool PMWssSubscriber::ParseBook(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    if (!FillBaseTs(body, recv_ts_ns, b.ts, DataSourceTsOrigin::kInferredFromDsTs)) return false;
...
bool PMWssSubscriber::ParseLastTrade(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    std::int64_t sz = 0;
    if (ExtractInt64(body, "size", sz)) t.size_micro = static_cast<std::uint64_t>(sz);
...
bool PMWssSubscriber::ParseSystemStatus(std::string_view body, std::int64_t recv_ts_ns, WssEvent& ev) {
    ...
    if (!FillBaseTs(body, recv_ts_ns, s.ts)) return false;
    if      (body.find("\"healthy\"")     != std::string_view::npos) s.health = SystemHealth::kHealthy;
    else if (body.find("\"degraded\"")    != std::string_view::npos) s.health = SystemHealth::kDegraded;
    else if (body.find("\"maintenance\"") != std::string_view::npos) s.health = SystemHealth::kMaintenance;
