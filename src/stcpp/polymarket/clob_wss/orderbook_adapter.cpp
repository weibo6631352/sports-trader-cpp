...
[[nodiscard]] inline double sum_depth(
    const std::array<stcpp::microstructure::OrderBookLevel, kAdapterBookDepth>& side,
    std::uint8_t n) noexcept {
    double s = 0.0;
    for (std::uint8_t i = 0; i < n && i < kAdapterBookDepth; ++i) {
        if (finite_pos(side[i].size_usdc))
            s += side[i].size_usdc;
    }
    return s;
}
...
void OrderBookAdapterState::FillSide(
    std::array<stcpp::microstructure::OrderBookLevel, kAdapterBookDepth>& dst, std::uint8_t& dst_n,
    const stcpp::microstructure::OrderBookLevel* src, std::uint8_t src_n) noexcept {
    dst_n = std::min(src_n, static_cast<std::uint8_t>(kAdapterBookDepth));
    for (std::uint8_t i = 0; i < dst_n; ++i)
        dst[i] = src[i];
    // zero-fill remaining slots
    for (std::uint8_t i = dst_n; i < kAdapterBookDepth; ++i)
        dst[i] = {};
}

void OrderBookAdapterState::UpsertLevel(
    std::array<stcpp::microstructure::OrderBookLevel, kAdapterBookDepth>& side, std::uint8_t& depth,
    const stcpp::microstructure::OrderBookLevel& lvl, bool ascending) noexcept {
    if (!detail::finite_pos(lvl.price))
        return;
    ...
    if (lvl.size_usdc <= 0.0)
        return;  // nothing to insert for a zero-size removal
    if (depth >= kAdapterBookDepth)
        return;  // no room (price too far from best)

    // Find insertion index (ascending: smallest price first; descending: largest first)
    std::uint8_t ins = depth;
    for (std::uint8_t i = 0; i < depth && i < kAdapterBookDepth; ++i) {
        bool insert_before = ascending ? (lvl.price < side[i].price) : (lvl.price > side[i].price);
        if (insert_before) {
            ins = i;
            break;
        }
...
OrderBookFeatures OrderBookAdapterState::OnSnapshot(const stcpp::microstructure::OrderBookLevel* bid_levels,
                                                    std::uint8_t bid_n,
                                                    const stcpp::microstructure::OrderBookLevel* ask_levels,
                                                    std::uint8_t ask_n,
                                                    const stcpp::microstructure::OrderBookTs& ts,
                                                    double tick_size, double last_trade_price,
...
OrderBookFeatures OrderBookAdapterState::OnDelta(const stcpp::microstructure::OrderBookLevel* bid_levels,
                                                 std::uint8_t bid_n,
                                                 const stcpp::microstructure::OrderBookLevel* ask_levels,
                                                 std::uint8_t ask_n,
                                                 const stcpp::microstructure::OrderBookTs& ts,
    ...
    if (detail::finite_pos(tick_size))
...
OrderBookFeatures OrderBookAdapterState::Compute(const stcpp::microstructure::OrderBookTs& ts,
    ...
    // Copy levels
    for (std::uint8_t i = 0; i < bid_depth_ && i < kAdapterBookDepth; ++i)
        out.bid[i] = bid_[i];
    for (std::uint8_t i = 0; i < ask_depth_ && i < kAdapterBookDepth; ++i)
        out.ask[i] = ask_[i];
    ...
    if (!detail::finite_pos(bb.price) || !detail::finite_pos(ba.price) || ba.price <= bb.price) {
...
OrderBookAdapter::OrderBookAdapter(IOrderBookFeatureSink* sink) noexcept : sink_(sink) {
    tokens_ = {};
}
...
bool OrderBookAdapter::RegisterToken(std::string_view token_id) noexcept {
    ...
    for (std::uint8_t i = 0; i < token_count_; ++i) {
        if (tokens_[i].token_id == token_id)
...
void OrderBookAdapter::ExtractBookLevels(const stcpp::polymarket::wss::OrderBookL2Update& book,
    ...
    tick_size_out =
        (book.tick_size_bps > 0) ? BpsToPrice(book.tick_size_bps) : stcpp::microstructure::TICK_01;

    // last_trade_price: from WssEvent field
    last_trade_price_out = (book.last_trade_price_bps > 0) ? BpsToPrice(book.last_trade_price_bps) : 0.0;
...
void OrderBookAdapter::ProcessEvent(std::string_view token_id,
                                    const stcpp::polymarket::wss::WssEvent& ev) noexcept {
    using stcpp::polymarket::wss::SubTopic;

    if (ev.topic != SubTopic::kBook && ev.topic != SubTopic::kPriceChange)
        return;
    ...
    ExtractBookLevels(book, bid_lvls, bid_n, ask_lvls, ask_n, tick_size, last_trade_price);

    OrderBookFeatures features{};
    if (book.is_snapshot) {
        ++snapshot_count_;
        features = entry->state.OnSnapshot(bid_lvls, bid_n, ask_lvls, ask_n, ts, tick_size, last_trade_price,
                                           ts.data_source_ts_ns);
    } else {
        ++delta_count_;
        features = entry->state.OnDelta(bid_lvls, bid_n, ask_lvls, ask_n, ts, tick_size);
    }
