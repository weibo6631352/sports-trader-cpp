...
OrderBookAdapter::OrderBookAdapter(OrderBookAdapterConfig cfg)
    : cfg_(cfg) {
...
bool OrderBookAdapter::OnEvent(const WssEvent&     ev,
...
void OrderBookAdapter::InjectMetadata(FeatureStoreBookRow& row,
...
AdapterSnapshotState OrderBookAdapter::GetSnapshotState(std::string_view token_id) const noexcept {
    auto it = token_states_.find(std::string(token_id));
    if (it == token_states_.end()) return AdapterSnapshotState::kWaitingSnapshot;
...
//
void OrderBookAdapter::ApplySnapshot(TokenState& st, const WssEvent& ev,
                                     std::int64_t recv_ts_ns) noexcept {
    ...
    if (b.mid_bps > 0) {
        ...
        // Clamp to (0, 1)
        if (st.bid_price <= 0.0) st.bid_price = st.tick_size;
        if (st.ask_price >= 1.0) st.ask_price = 1.0 - st.tick_size;
        ...
        if (b.spread_bps > 0) {
            ...
            double mid_proxy = BpsToPrice(b.last_trade_price_bps > 0
                                          ? b.last_trade_price_bps
                                          : 5000u);
...
//
void OrderBookAdapter::ApplyDelta(TokenState& st, const WssEvent& ev,
                                  std::int64_t recv_ts_ns) noexcept {
    const auto& b = ev.payload.book;
...
void OrderBookAdapter::FillSnapshot(const TokenState&  st,
                                    std::string_view   /*token_id*/,
    ...
    if (IsValidPrice(st.bid_price) && IsValidPrice(st.ask_price) &&
        st.ask_price > st.bid_price) {
        double mid = 0.5 * (st.bid_price + st.ask_price);
        snap.spread_bps = static_cast<std::int32_t>(
            (st.ask_price - st.bid_price) / mid * 10000.0 + 0.5);
...
void OrderBookAdapter::FillBookRow(const OrderBookSnapshot& snap,
