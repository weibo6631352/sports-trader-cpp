...
// ---------------------------------------------------------------------------
ClobFillOutput FillRateModel::compute_from_clob_book(OrderBookSnapshot const& book, Microprobe const& probe,
                                                     FillIntent const& intent) noexcept {
    ClobFillOutput out;
    out.ts = book.ts;
    ...
    //
    double const top3 = (book.top3_depth_usdc > 0.0)
                            ? book.top3_depth_usdc
