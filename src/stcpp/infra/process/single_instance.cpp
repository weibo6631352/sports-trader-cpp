...
// ---------------------------------------------------------------------------
bool WritePidFile(int fd, std::int64_t pid, std::int64_t start_ts_ns, std::string_view exec_mode,
    ...
    int len = ::snprintf(buf, sizeof(buf), "%lld\n%lld\n%.*s\n%.*s\n", static_cast<long long>(pid),
                         static_cast<long long>(start_ts_ns), static_cast<int>(exec_mode.size()),
                         exec_mode.data(), static_cast<int>(commit.size()), commit.data());
    ...
    *nl = '\0';
    {
        ...
        if (ec == std::errc{})
            c.pid = static_cast<std::int64_t>(tmp);
    }
    p = nl + 1;
    ...
    *nl = '\0';
    {
        ...
        auto [ptr, ec] = std::from_chars(p, nl, tmp);
        if (ec == std::errc{})
            c.start_ts_ns = static_cast<std::int64_t>(tmp);
    }
    p = nl + 1;
...
std::string SingleInstanceLock::path_for(stcpp::execution::ExecutionMode mode) {
    ...
    char pid_dir_buf[64];
    if (!env_override) {
        ::snprintf(pid_dir_buf, sizeof(pid_dir_buf), "/tmp/stcpp_test_%d", static_cast<int>(::getpid()));
...
SingleInstanceLock::SingleInstanceLock(stcpp::execution::ExecutionMode mode) {
    ...
    const std::string pid_dir =
        (last_slash != std::string::npos) ? pid_path_.substr(0, last_slash) : std::string(STCPP_PID_DIR);

    if (!MkdirP(pid_dir.c_str())) {
        throw SingleInstanceLockFailure(std::string("[single-instance] FATAL: cannot create pid dir: ") +
    ...
    fd_guard_.reset(::open(pid_path_.c_str(),
                           ...
                           0644));
    if (!fd_guard_.valid()) {
        throw SingleInstanceLockFailure(std::string("[single-instance] FATAL: open(") + pid_path_ +
    ...
    if (::flock(fd_guard_.get(), LOCK_EX | LOCK_NB) != 0) {  // NOLINT(hicpp-signed-bitwise)
        if (errno == EWOULDBLOCK) {
            ...
            throw SingleInstanceLockFailure(
                std::string("[single-instance] FATAL: ") + std::string(stcpp::execution::ToString(mode)) +
                    " already running pid=" + std::to_string(c.pid) +
                    " since=" + std::to_string(c.start_ts_ns) + " commit=" + c.commit,
        ...
        throw SingleInstanceLockFailure(
            std::string("[single-instance] FATAL: flock failed errno=") + std::to_string(saved), 0, 0,
            std::string(stcpp::execution::ToString(mode)), STCPP_BUILD_COMMIT);
    }

    // 5. 写 PID file 内容
    const std::int64_t start_ts = NowRealtimeNs();
    WritePidFile(fd_guard_.get(), static_cast<std::int64_t>(::getpid()), start_ts,
                 stcpp::execution::ToString(mode), STCPP_BUILD_COMMIT);
...
void InstallSigtermHandler(std::string pid_path) noexcept {
    ...
    const std::size_t copy_len = pid_path.size() < kMaxPidPathLen - 1 ? pid_path.size() : kMaxPidPathLen - 1;
