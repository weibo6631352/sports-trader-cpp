...
void MakeAuditId(std::array<std::uint8_t, 16>& out) noexcept {
    ...
    const std::uint64_t ts_ms =
        static_cast<std::uint64_t>(t.tv_sec) * 1000ULL + static_cast<std::uint64_t>(t.tv_nsec) / 1'000'000ULL;
...
IngestRawWriter::IngestRawWriter(IngestRawWriterConfig cfg) : cfg_(cfg) {
...
ApplyResult IngestRawWriter::record_frame(IngestSourceKind source_kind, std::string_view payload_text,
    ...
    return record_frame(source_kind, std::span<const std::byte>(p, payload_text.size()), event_ts_ns,
                        data_source_ts_ns);
}
...
ApplyResult IngestRawWriter::record_frame(IngestSourceKind source_kind, std::span<const std::byte> payload,
                                          std::int64_t event_ts_ns, std::int64_t data_source_ts_ns) noexcept {
...
bool IngestRawWriter::push_tier1(const IngestRawRecord& rec) noexcept {
    ...
    if (tier1_ring_full_.load(std::memory_order_relaxed))
        return false;
    (void)rec;
    return true;
}
...
bool IngestRawWriter::push_tier2(const IngestRawRecord& rec) noexcept {
    ...
    if (tier2_ring_full_.load(std::memory_order_relaxed))
        return false;
    (void)rec;
    return true;
}

bool IngestRawWriter::check_pit(std::int64_t event_ts, std::int64_t ds_ts, std::int64_t ingest_ts,
