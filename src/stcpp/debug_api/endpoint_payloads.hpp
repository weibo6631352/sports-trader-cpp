...
inline std::string account(const StateProvider& sp) {
    ...
    b += ",\"engine_pnl\":{\"sharp\":{\"realized\":";
    b += json::num(a.eng_sharp_realized);
    b += ",\"settles\":";
    b += json::i64(a.eng_sharp_settles);
    b += ",\"wins\":";
    b += json::i64(a.eng_sharp_wins);
    b += "},\"flb\":{\"realized\":";
    b += json::num(a.eng_flb_realized);
    b += ",\"settles\":";
    b += json::i64(a.eng_flb_settles);
    b += ",\"wins\":";
    b += json::i64(a.eng_flb_wins);
    b += "},\"flb-dip\":{\"realized\":";
    b += json::num(a.eng_dip_realized);
    b += ",\"settles\":";
    b += json::i64(a.eng_dip_settles);
    b += ",\"wins\":";
    b += json::i64(a.eng_dip_wins);
    b += "}}";
    b += ",\"position_mtm\":";
...
inline std::string positions(const StateProvider& sp, std::int64_t as_of_ns = -1) {
    ...
    for (std::size_t i = 0; i < rows.size(); ++i) {
        ...
        b += ",\"status\":";
        b += json::str(r.status);  // 状态标 (2026-06-11): live/settling_won/settling_lost/settling
        b += ",\"as_of_ts\":";
...
inline std::string fills(const StateProvider& sp, const std::string& market = "") {
    ...
    for (std::size_t i = 0; i < rows.size(); ++i) {
        ...
        b += json::num(r.fee);
        b += ",\"exit_reason\":";
        b += json::str(r.exit_reason);  // 卖出原因 (老板「出现卖出就检查是否合理」)
        b += ",\"engine\":";
        b += json::str(r.engine);  // 引擎标签 (FLB 并跑对比, 2026-06-11)
        b += ",\"as_of_ts\":";
        b += json::i64(r.as_of_ts_ns);
        b += '}';
    }
