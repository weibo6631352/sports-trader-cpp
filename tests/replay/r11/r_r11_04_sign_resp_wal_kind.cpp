...
TEST_F(RR1104Fixture, R_R11_04_sign_response_wal_kind_replay_all_paper_audit) {
    ...
    for (int i = 0; i < kN; ++i) {
        ...
        if (out.went_through_signer) {
            ...
            const bool sign_ok = r11.on_decision_wal_kind(
                out.sign_resp.audit_wal_kind, static_cast<std::uint64_t>(i + 1));
            if (sign_ok) ++sign_wal_ok;

            // VirtualFill.audit_wal_kind replay 校验
            const bool fill_ok = r11.on_fill_wal_kind(
                out.fill.audit_wal_kind, static_cast<std::uint64_t>(i + 1));
            if (fill_ok) ++fill_wal_ok;
        }
    }

    EXPECT_GE(sign_through, 1) << "R-R11-04: 至少 1 笔走 signer";
    EXPECT_EQ(sign_through, sign_wal_ok)
        << "R-R11-04 / R-11: 全部 SignResponse.audit_wal_kind == PaperAudit";
    EXPECT_EQ(sign_through, fill_wal_ok)
        << "R-R11-04 / R-11: 全部 VirtualFill.audit_wal_kind == PaperAudit";
