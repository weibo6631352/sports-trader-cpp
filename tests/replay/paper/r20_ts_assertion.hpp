...
public:
    ...
    bool on_event(std::uint64_t seq, std::int64_t event_ts, std::int64_t data_source_ts,
                  std::int64_t ingestion_ts, std::int64_t as_of_ts) {
        bool ok = true;

        // event_ts == 0 (禁用本地 now() 替代上游 ts)
        if (event_ts <= 0) {
            ++violation_count_;
            violations_.push_back({seq, "event_ts == 0 (禁用本地 now() 替代上游 ts)", event_ts,
                                   data_source_ts, ingestion_ts, as_of_ts});
            ok = false;
        }
        ...
        if (data_source_ts < event_ts) {
            ...
            violations_.push_back(
                {seq, "data_source_ts < event_ts", event_ts, data_source_ts, ingestion_ts, as_of_ts});
            ok = false;
        }
        ...
        if (ingestion_ts < data_source_ts) {
            ++violation_count_;
            violations_.push_back(
                {seq, "ingestion_ts < data_source_ts", event_ts, data_source_ts, ingestion_ts, as_of_ts});
            ok = false;
        }
        ...
        if (as_of_ts < ingestion_ts) {
            ++violation_count_;
            violations_.push_back(
                {seq, "as_of_ts < ingestion_ts", event_ts, data_source_ts, ingestion_ts, as_of_ts});
            ok = false;
        }
    ...
    [[nodiscard]] const std::vector<R20ViolationRecord>& violations() const noexcept { return violations_; }

    // 记录 INFERRED_FROM_INGESTION (不 FAIL, 月度 sweep 用)
    void note_inferred_ts(std::uint64_t /*seq*/) noexcept { ++inferred_ts_count_; }

    // dump 违例到 string (可读输出, spec §1.1 "diff 必须可读")
    [[nodiscard]] std::string dump_violations() const {
        std::string out;
        for (const auto& v : violations_) {
            out += "[R-20 violation seq=" + std::to_string(v.seq) + " type=" + v.violation_type +
                   " event_ts=" + std::to_string(v.event_ts) + " ds_ts=" + std::to_string(v.data_source_ts) +
                   " ing_ts=" + std::to_string(v.ingestion_ts) + " as_of_ts=" + std::to_string(v.as_of_ts) +
                   "]\n";
        }
        return out;
    }
