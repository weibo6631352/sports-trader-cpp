...
public:
    ...
    bool on_decision(std::uint64_t seq, stcpp::observability::AuditEventType event_type,
                     std::int64_t decision_ts, const Hash256& emitter_current_hash) {
        const auto digest = stcpp::observability::Blake3Hasher::compute_payload_hash(
            seq, static_cast<std::uint8_t>(event_type), decision_ts);

        const auto expected_current = stcpp::observability::Blake3Hasher::hash_chain(mirror_prev_, digest);
        ...
        if (expected_current != emitter_current_hash) {
            violations_.push_back(
                {seq, expected_current, emitter_current_hash,
                 "R-audit-chain: replay decision #" + std::to_string(seq) + " chain head mismatch"});
            return false;
        }
        return true;
    }
    ...
    [[nodiscard]] bool finalize() {
        ...
        if (has_golden_) {
            ...
            if (mirror_prev_ != golden_chain_head_) {
                violations_.push_back({emitted_count_, mirror_prev_, golden_chain_head_,
    ...
    [[nodiscard]] const std::vector<ChainViolationRecord>& violations() const noexcept { return violations_; }
