...
TEST(R20Replay30Day, R_R20_07_paper_30d_attestation_zero_violations) {
    ...
    EXPECT_TRUE(ok) << "R-R20-07: 30 日 paper run 0 违例 (小余签字前提)";
    EXPECT_EQ(r20.violation_count(), 0u) << "R-R20-07: violation_count == 0 (attestation 通过)";
    ...
    EXPECT_EQ(r20.inferred_ts_count(), 0u) << "R-R20-07: 无推断 ts (stub 数据全部合法)";
}
