...
TEST(R20ReplayWalHeader, R_R20_06_wal_header_payload_ts_match) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        const bool ok = r20.on_event(
            ...
            rec.event_ts, rec.data_source_ts, rec.ingestion_ts, rec.as_of_ts);
        EXPECT_TRUE(ok) << "R-R20-06: record " << i << " R-20 校验通过";
    }

    // finalize: 0 违例
    EXPECT_TRUE(r20.finalize())
        << "R-R20-06: WAL header/payload 双轨校验 0 违例";
    EXPECT_EQ(r20.violation_count(), 0u);
    EXPECT_EQ(writer->HighWatermark(), 10u)
        << "R-R20-06: WAL HighWatermark == 10 (10 条写入)";
}
