...
TEST(AuditChainReplay, R_chain_01_20_records_chain_assertion_pass) {
    ...
    for (int i = 0; i < kN; ++i) {
        ...
        const bool chain_ok = chain_assert.on_decision(
            seq, AuditEventType::OrderApproved, ctx.decision_ts, emitter.last_hash());

        EXPECT_TRUE(chain_ok)
            << "R-chain-01: replay chain head 第 " << i << " 笔不匹配";
    }
