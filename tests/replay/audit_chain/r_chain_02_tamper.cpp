...
TEST(AuditChainReplay, R_chain_02_tamper_detected_by_replay_assertion) {
    ...
    struct Snap {
        ...
        stcpp::observability::Blake3Hasher::Hash256 chain_after;
    };
        ...
        for (std::size_t i = 0; i < static_cast<std::size_t>(kN); ++i) {
            const bool ok =
                chain_assert.on_decision(static_cast<std::uint64_t>(i + 1), AuditEventType::OrderApproved,
                                         snaps[i].ctx.decision_ts, snaps[i].chain_after);
    ...
    // 篡改第 10 条 decision_ts: 修改 snaps[10].ctx.decision_ts + 1
    {
        ...
        for (std::size_t i = 0; i < static_cast<std::size_t>(kN); ++i) {
            ...
            const bool ok =
                chain_tamper.on_decision(static_cast<std::uint64_t>(i + 1), AuditEventType::OrderApproved, ts,
        ...
        EXPECT_TRUE(first_mismatch) << "R-chain-02 / M1-A06: 篡改任意 record 必被 chain replay 检出";
        EXPECT_EQ(first_mismatch_seq, static_cast<std::uint64_t>(10 + 1))
            << "R-chain-02 / M1-A06: chain 首次不一致定位到被篡改 record (seq=11)";
        EXPECT_GE(chain_tamper.violation_count(), 1u) << "R-chain-02: 篡改后 violation_count >= 1";
    }
}
