...
ms::FillIntent MakeIntent(double size_usdc, ms::Sport sport = ms::Sport::Basketball,
                          ms::InplayPhase phase = ms::InplayPhase::Pregame, double price = 0.50) {
    ms::FillIntent it;
    it.side = ms::Side::Buy;
    it.price = price;
...
ex::VirtualOrderWithBook MakeOrderWithBook(double size_usdc = 200.0, double l1_size = 1500.0,
                                           double top3 = 5000.0, std::int32_t spread = 20,
                                           double price = 0.50) {
    ...
    o.probe = MakeProbe();
    o.fill_intent = MakeIntent(size_usdc, ms::Sport::Basketball, ms::InplayPhase::Pregame, price);
...
TEST(FillRateModelV02_Clob, T06_SlippageConservativeEstimate) {
    ...
    auto probe = MakeProbe();
    auto intent = MakeIntent(100.0, ms::Sport::Basketball, ms::InplayPhase::Pregame, 0.50);
    auto out = ms::FillRateModel::compute_from_clob_book(book, probe, intent);
...
TEST(FillRateModelV02_Clob, T20_SumPenaltiesFormula) {
    ...
    double const expected_sum =
        -(out.spread_penalty + out.adverse_penalty + out.time_decay_penalty) + out.sport_bias;
