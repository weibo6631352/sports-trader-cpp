...
TEST(PMClientAbi, T1_InterfaceHashTable) {
    ...
    constexpr std::array<std::pair<std::string_view, std::string_view>, 14> kAbiTable = {{
        ...
        {"F-14", "cb14b44cb37ac44f"},  // SubscribeSportsWss(vector<string>,OrderBookCallback,void*)|Result<uint32_t>
    }};

    // 验证表条目数 (14 不变)
    ASSERT_EQ(kAbiTable.size(), 14u)
        << "ABI lock: F-01~F-14 必须 14 条 (添加接口走 L0, 不改本表)";

    // 验证 F-ID 序号连续 (F-01 ~ F-14)
    for (std::size_t i = 0; i < kAbiTable.size(); ++i) {
        char expected[5];
        std::snprintf(expected, sizeof(expected), "F-%02zu", i + 1);
        EXPECT_EQ(kAbiTable[i].first, std::string_view(expected))
            << "ABI lock: 表序号不连续, index=" << i;
    }

    // 验证 hash 非空 + 16 char
    for (const auto& [fid, hash] : kAbiTable) {
        EXPECT_EQ(hash.size(), 16u)
            << "ABI lock: " << fid << " hash 必须 16 hex char";
        EXPECT_FALSE(hash.empty())
            << "ABI lock: " << fid << " hash 不能为空";
    }
}
...
TEST(PMClientAbi, T2_TimestampQuad_SizeAndLayout) {
    ...
    EXPECT_EQ(sizeof(TimestampQuad), 40u)
        << "ABI lock: TimestampQuad sizeof — L2 改动需 老李+老孙+GM 三方签";

    // 4 ts 字段 offset 严格锁定 (R-20 IPC 契约 + 老孙 signer v5.1 §5.4)
    EXPECT_EQ(offsetof(TimestampQuad, event_ts_ns),        0u)
        << "ABI lock: TimestampQuad.event_ts_ns offset=0";
    ...
    EXPECT_EQ(offsetof(TimestampQuad, as_of_ts_ns),       24u)
        << "ABI lock: TimestampQuad.as_of_ts_ns offset=24";
    EXPECT_EQ(offsetof(TimestampQuad, ds_ts_source),      32u)
        << "ABI lock: TimestampQuad.ds_ts_source offset=32";
...
TEST(PMClientAbi, T2_SignedOrder_FieldPresence) {
    ...
    SignedOrder o{};
    EXPECT_EQ(o.signature_type, 1u)
        << "ABI lock: SignedOrder.signature_type 默认必须 = 1 (HMAC bug #3)";
    EXPECT_EQ(o.side, 0u)
        << "ABI lock: SignedOrder.side 默认 0 (BUY YES)";
    EXPECT_EQ(o.limit_price_bps, 0u);
    EXPECT_EQ(o.size_usdc_micro, 0u);
    EXPECT_EQ(o.expiration_unix_s, 0);
    EXPECT_TRUE(o.signature.empty())
        << "ABI lock: SignedOrder.signature paper 留空 (live reserve)";
...
TEST(PMClientAbi, T2_OrderAck_FieldPresence) {
    ...
    OrderAck ack{};
    EXPECT_EQ(ack.status, OrderStatus::Booked)
        << "ABI lock: OrderAck.status 默认 Booked";
...
TEST(PMClientAbi, T4_TimestampQuad_PitChainSemantics) {
    ...
    EXPECT_LE(ts.event_ts_ns,       ts.data_source_ts_ns)
        << "R-20: event_ts <= data_source_ts";
    EXPECT_LE(ts.data_source_ts_ns, ts.ingestion_ts_ns)
        << "R-20: data_source_ts <= ingestion_ts";
    EXPECT_LE(ts.ingestion_ts_ns,   ts.as_of_ts_ns)
        << "R-20: ingestion_ts <= as_of_ts";
