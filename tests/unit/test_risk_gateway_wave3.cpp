...
TEST_F(Wave3Test, D1_DD_SoftThreshold_CloseApproved) {
    ...
    auto d = rm_->evaluate(it);
    EXPECT_EQ(d.decision, Decision::APPROVED)
        << "D1: 软熔断区间内, is_close=true 平仓单应放行";
}
...
TEST_F(Wave3Test, D1_DD_HardKill_HaltedState) {
    ...
    EXPECT_EQ(rm_->state(), RmState::HALTED)
        << "D1: -5% 硬 kill 后 RmState 必须转为 HALTED";
}
...
TEST_F(Wave3Test, D2_TS_V2_MISSING_zero) {
    ...
    EXPECT_EQ(d.sub_reason, InvalidIntentSubReason::TS_V2_MISSING)
        << "D2: timestamp_ms=0 应拒 TS_V2_MISSING";
}
...
TEST_F(Wave3Test, D2_TS_V2_boundary_59s_approved) {
    ...
    auto d = rm_->evaluate(it);
    EXPECT_EQ(d.decision, Decision::APPROVED)
        << "D2: timestamp_ms 在 60s 窗口内 (59s 前), 应 APPROVED";
}
...
TEST_F(Wave3Test, D2_TS_V2_FUTURE_4s_approved) {
    ...
    auto d = rm_->evaluate(it);
    EXPECT_EQ(d.decision, Decision::APPROVED)
        << "D2: timestamp_ms 比 now 早 4s (在 5s 容忍内), 应 APPROVED";
}
...
TEST_F(Wave3Test, D3_Bytes32_custom_valid_hex) {
    ...
    auto d = rm_->evaluate(it);
    EXPECT_EQ(d.decision, Decision::APPROVED)
        << "D3: 合法小写 bytes32 hex, 应 APPROVED";
}
...
TEST_F(Wave3Test, D4_Fee_EdgeErodedByFee_Rejected) {
    ...
    auto d = rm_->evaluate(it);
    EXPECT_EQ(d.decision, Decision::REJECTED)
        << "D4: edge_ci_lower=0.005 被 fee=0.0075 侵蚀, 应 REJECTED";
...
TEST_F(Wave3Test, D4_Fee_ExactBoundary_Rejected) {
    ...
    EXPECT_EQ(d.decision, Decision::REJECTED)
        << "D4: net_edge = 0 (= floor 0.0), 应 REJECTED (R-fee-2 边界)";
}
...
TEST_F(Wave3Test, D4_Fee_IsolationCheck) {
    ...
    EXPECT_DOUBLE_EQ(it.price, original_price)
        << "D4 isolation: intent.price 不应被 fee 计算改写";
...
TEST_F(Wave3Test, D5_AuditTransparency_Approved_TimestampMs) {
    ...
    EXPECT_EQ(emitter_->back().timestamp_ms, ts_ms)
        << "D5: APPROVED 路径 audit record 必须透传 timestamp_ms";
}
...
TEST_F(Wave3Test, D5_AuditTransparency_Approved_Metadata) {
    ...
    EXPECT_EQ(d.decision, Decision::APPROVED);
    ASSERT_EQ(emitter_->size(), 1u);
    EXPECT_EQ(emitter_->back().metadata, it.metadata)
        << "D5: APPROVED 路径 audit record 必须透传 metadata";
}
...
TEST_F(Wave3Test, D5_AuditTransparency_Approved_Builder) {
    ...
    EXPECT_EQ(d.decision, Decision::APPROVED);
    ASSERT_EQ(emitter_->size(), 1u);
    EXPECT_EQ(emitter_->back().builder, it.builder)
        << "D5: APPROVED 路径 audit record 必须透传 builder";
}
...
TEST_F(Wave3Test, D5_AuditTransparency_Rejected_V2Fields) {
    ...
    ASSERT_EQ(emitter_->size(), 1u);
    EXPECT_EQ(emitter_->back().timestamp_ms, ts_ms)
        << "D5: REJECTED 路径 audit record 必须透传 timestamp_ms";
    EXPECT_EQ(emitter_->back().metadata, it.metadata)
        << "D5: REJECTED 路径 audit record 必须透传 metadata";
    EXPECT_EQ(emitter_->back().builder, it.builder)
        << "D5: REJECTED 路径 audit record 必须透传 builder";
}
...
TEST_F(Wave3Test, D5_AuditTransparency_DefaultZeroValues) {
    ...
    EXPECT_EQ(d.decision, Decision::APPROVED);
    ASSERT_EQ(emitter_->size(), 1u);
    EXPECT_EQ(emitter_->back().metadata, kBytes32Zero)
        << "D5: bytes32(0) 默认 metadata 应正确透传";
    EXPECT_EQ(emitter_->back().builder, kBytes32Zero)
        << "D5: bytes32(0) 默认 builder 应正确透传";
}
