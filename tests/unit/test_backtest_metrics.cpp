...
TradeRecord make_settled(TradeSide side, SettleOutcome outcome,
                          double fill_price, double size,
                          std::int64_t as_of_ns,
                          bool filtered = false) {
    TradeRecord t;
    ...
    t.realized_pnl_usdc = compute_realized_pnl(
        side, outcome, fill_price, size, t.fee_rate, t.slippage_rate);
    return t;
}
...
TEST(MetricsBasic, AllWin) {
    std::vector<TradeRecord> trades;
    for (int i = 0; i < 5; ++i) {
        trades.push_back(make_settled(
            TradeSide::BuyYes, SettleOutcome::YesWins,
            0.45, 100.0,
...
TEST(MetricsDaily, DailyReturns) {
    ...
    std::vector<TradeRecord> trades;
    // day 0 (as_of = 0.5 day):
    trades.push_back(make_settled(TradeSide::BuyYes, SettleOutcome::YesWins,
                                   0.45, 100.0, kDayNs / 2));
    // day 1 (as_of = 1.5 day):
    trades.push_back(make_settled(TradeSide::BuyYes, SettleOutcome::NoWins,
                                   0.45, 100.0, kDayNs + kDayNs / 2));
    // day 2 (as_of = 2.5 day):
    trades.push_back(make_settled(TradeSide::BuyYes, SettleOutcome::YesWins,
                                   0.45, 100.0, 2LL * kDayNs + kDayNs / 2));
