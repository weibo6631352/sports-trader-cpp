...
constexpr std::int64_t kNowNs =
    1'700'000'000LL * kNs;  // 任取一个未来值, 但 < real now() — wait, 不能用 NowRealtimeNs (会比真实小).
...
TEST(GateEvaluator_G2_StageThresholds, M2_Threshold_Is_0_3) {
    ...
    EXPECT_DOUBLE_EQ(kG2SharpeCILowerByStage[static_cast<std::size_t>(GateStage::M2)], kExpected)
...
TEST(GateEvaluator_G2_StageThresholds, M4_5_Threshold_Is_0_5) {
    ...
    EXPECT_DOUBLE_EQ(kG2SharpeCILowerByStage[static_cast<std::size_t>(GateStage::M4_5)], kExpected)
        << "M4.5 stage threshold must be 0.5 (ADR-016, 小梁会签 SSOT)";
    // 同时验证 kG2_SharpeCILow 引用 M4_5 slot
    EXPECT_DOUBLE_EQ(kG2_SharpeCILow, kExpected) << "kG2_SharpeCILow must equal M4_5 slot (0.5)";
}
...
TEST(GateEvaluator_G2_StageThresholds, M5_Q1_Threshold_Is_0_8) {
    ...
    EXPECT_DOUBLE_EQ(kG2SharpeCILowerByStage[static_cast<std::size_t>(GateStage::M5_Q1)], kExpected)
...
TEST(GateEvaluator_G2_StageThresholds, NorthStar_Threshold_Is_1_5) {
    ...
    EXPECT_DOUBLE_EQ(kG2SharpeCILowerByStage[static_cast<std::size_t>(GateStage::NorthStar)], kExpected)
