...
static RiskDecisionInput make_v13_input(
...
TEST(AuditSchemaV13, T1_TokenId_RoundTrip) {
    ...
    EXPECT_STREQ(rec2.condition_id.data(), kCondId.data())
        << "T1: condition_id round-trip failed";
    ...
    ASSERT_TRUE(r.has_value()) << "T1: emit_decision failed: "
...
TEST(AuditSchemaV13, T2_Outcome_AllValues_Serialized) {
    ...
    for (std::size_t i = 0; i < 7; ++i) {
        auto in = make_v13_input(kTokenIds[i], kOutcomes[i], 0,
                                 AuditEventType::OrderApproved);
        auto r = em.emit_decision(in);
        ASSERT_TRUE(r.has_value())
            << "T2: emit_decision outcome=" << static_cast<int>(kOutcomes[i])
...
TEST(AuditSchemaV13, T4_BLAKE3_Chain_NewFields_NotBreak) {
    ...
    ASSERT_TRUE(em.emit_decision(ctx1).has_value()) << "T4: emit seq=2 failed";
    {
        ...
        const auto expected = Blake3Hasher::hash_chain(mirror_prev, payload);
        EXPECT_EQ(em.last_hash(), expected)
            << "T4: seq=2 chain hash != local recompute";
        mirror_prev = expected;
    }
    ...
    ASSERT_TRUE(em.emit_decision(ctx2).has_value()) << "T4: emit seq=3 failed";
    {
        ...
        const auto expected = Blake3Hasher::hash_chain(mirror_prev, payload);
        EXPECT_EQ(em.last_hash(), expected)
            << "T4: seq=3 chain hash != local recompute";
    }
    ...
    EXPECT_TRUE(em.hash_chain_verify(1, 3))
        << "T4: hash_chain_verify(1,3) 应通过 (v1.3 新字段不破 chain)";
}
...
TEST(AuditSchemaV13, T5_V12_To_V13_Migration) {
    ...
    // Case B: v1.2 record, is_buy=false → migration 后 side=Sell(1)
    {
        ...
        EXPECT_EQ(rec_v12.outcome, static_cast<std::uint8_t>(0))
            << "T5-B: migration default outcome=Yes(0)";
        EXPECT_EQ(rec_v12.token_id[0], '\0')
...
static_assert(sizeof(AuditRecord) <= 65535,
              "AuditRecord v1.3 单条 ≤ u16 LEN (framework 约束)");
