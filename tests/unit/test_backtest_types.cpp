...
TEST(BacktestTypes, RealizePnlBuyYesWin) {
    ...
    //    = 600 - 13.2 = 586.8
    double const pnl = compute_realized_pnl(
...
TEST(BacktestTypes, RealizePnlBuyYesLose) {
    ...
    //     = -400 - 13.2 = -413.2
    double const pnl = compute_realized_pnl(
        TradeSide::BuyYes, SettleOutcome::NoWins,
        fill, size, fee, slip);
...
TEST(BacktestTypes, RealizePnlSellYesWin) {
    ...
    //    = 400 - 19.8 = 380.2
    double const pnl = compute_realized_pnl(
        TradeSide::SellYes, SettleOutcome::NoWins,
        fill, size, fee, slip);
...
TEST(BacktestTypes, PendingOutcomePnlZero) {
    double const pnl = compute_realized_pnl(
        TradeSide::BuyYes, SettleOutcome::Pending,
        0.5, 1000.0, 0.03, 0.003);
