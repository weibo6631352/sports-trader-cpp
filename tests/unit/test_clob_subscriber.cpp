...
public:
    ...
    void Close() override {
        if (connected_) {
            connected_ = false;
            if (on_disconnected_) on_disconnected_("close");
        }
    }
    ...
    void Inject(std::string_view payload, std::int64_t recv_ts_ns) {
        if (on_text_frame_) on_text_frame_(payload, recv_ts_ns);
    }
...
struct TestFixture {
    ...
    explicit TestFixture(PolymarketCLOBSubscriberConfig cfg = MakeDefaultCfg(),
                         std::size_t market_cap = 256,
        ...
        sub = std::make_unique<PolymarketCLOBSubscriber>(
            std::move(mt), std::move(ut),
            market_sink, user_sink,
            std::move(cfg));
    }
};
...
TEST(PolymarketCLOBSubscriber, T1_SubscribePayloadDualToken) {
    ...
    const std::string& no_token =
        "40471602955768311660697606657820773573478994022447498274937726738952879800738";
    EXPECT_NE(subscribe_frame->find(yes_token), std::string::npos)
        << "YES token must be in assets_ids";
    EXPECT_NE(subscribe_frame->find(no_token), std::string::npos)
        << "NO token must be in assets_ids";
    ...
    EXPECT_EQ(fx.market_tp->last_connect_url_,
              "wss://ws-subscriptions-clob.polymarket.com/ws/market");
    EXPECT_EQ(fx.user_tp->last_connect_url_,
...
TEST(PolymarketCLOBSubscriber, T2_BookEventParseAndFourTs) {
    ...
    ASSERT_EQ(fx.market_sink->events_.size(), 1u)
        << "Expected 1 book event in market sink";
    ...
    EXPECT_EQ(ts.event_ts_ns, ts.data_source_ts_ns)
        << "event_ts must equal data_source_ts for book events";
    ...
    EXPECT_EQ(ts.ingestion_ts_ns, kRecvTs)
        << "ingestion_ts must equal recv_ts_ns from transport callback";

    // Verify tick_size and last_trade_price extraction
    // tick_size "0.01" → 100 bps
    EXPECT_EQ(ev.payload.book.tick_size_bps, 100u)
        << "tick_size 0.01 should map to 100 bps";
    // last_trade_price "0.545" → 5450 bps
    EXPECT_EQ(ev.payload.book.last_trade_price_bps, 5450u)
        << "last_trade_price 0.545 should map to 5450 bps";
...
TEST(PolymarketCLOBSubscriber, T3_ReconnectOnDisconnect) {
    ...
    ASSERT_GE(frames_before_disconnect, 1u)
        << "Must send subscribe frame on initial connect";
    ...
    EXPECT_GE(fx.market_tp->connect_calls_, 2)
        << "AsyncConnect must be called again after disconnect";
    ...
    ASSERT_NE(resent_frame, nullptr)
        << "Resent frame after reconnect must contain assets_ids";
    ...
    EXPECT_GE(fx.market_sink->events_.size(), 1u)
        << "book event after reconnect must be accepted";
