...
static KP gen() { KP k; crypto_sign_ed25519_keypair(k.pk.data(), k.sk.data()); return k; }
...
class SUT : public ::testing::Test {
 protected:
    ...
    void SetUp() override {
        kh = gen(); kt = gen(); kl = gen();
        now = std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::system_clock::now().time_since_epoch()).count();
        payload = stcpp::cli::ThreeSignatureVerifier::build_payload("s1", "deadbeef" "01234567" "deadbeef" "01234567", now);
    }
    KP kh, kt, kl;
    std::int64_t now{};
    std::string  payload;
...
TEST_F(SUT, T1_ThreeSig_OK) {
    ...
    in.laohan=mksig(kh,payload,now); in.laotang=mksig(kt,payload,now); in.laolei=mksig(kl,payload,now);
    in.payload=payload;
...
TEST_F(SUT, T2_SingleSig_Rejected) {
    stcpp::cli::ThreeSignatureInput in;
    in.laohan.sig_timestamp_ns=now; in.laotang.sig_timestamp_ns=now;
    in.laolei=mksig(kl,payload,now); in.payload=payload;
...
TEST_F(SUT, T4_WrongKey_Rejected) {
    ...
    unsigned long long len=0;
    crypto_sign_ed25519_sign_detached(bad.signature.data(), &len,
        reinterpret_cast<const unsigned char*>(payload.data()),
    ...
    stcpp::cli::ThreeSignatureInput in;
    in.laohan=mksig(kh,payload,now); in.laotang=bad; in.laolei=mksig(kl,payload,now);
    in.payload=payload;
    EXPECT_EQ(V.verify(in,now), stcpp::cli::ThreeSigResult::LAOTANG_FAILED);
}
...
TEST_F(SUT, T5_ExpiredTs_Rejected) {
    ...
    std::string old_payload = stcpp::cli::ThreeSignatureVerifier::build_payload("s1","aabbccdd11223344aabbccdd11223344",old_ts);
...
TEST_F(SUT, T6_AuditIds_Unique) {
    auto mk = [](std::int64_t ts, std::uint8_t seq) {
        ...
        id[8]=seq; id[9]=static_cast<std::uint8_t>(ts&0xFF); return id;
    };
    auto a1=mk(now,1), a2=mk(now,2), a3=mk(now,3);
    EXPECT_NE(a1,(std::array<std::uint8_t,16>{}));
    EXPECT_NE(a1,a2); EXPECT_NE(a2,a3); EXPECT_NE(a1,a3);
...
TEST_F(SUT, T7_ObsWindow_ReTrigger) {
    ...
    i.event_ts_ns=now; i.data_source_ts_ns=now; i.ingestion_ts_ns=now; i.as_of_ts_ns=now;
    i.market_id="mkt"; i.strategy_id="s1"; i.signal_id="sig1";
    i.is_buy=true; i.price=0.45; i.size_usdc=100;
    i.tick_size=0.01; i.book_depth_l1_usdc=10000.0; i.book_snapshot_ts_ns=now;
