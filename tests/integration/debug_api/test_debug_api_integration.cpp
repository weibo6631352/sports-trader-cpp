...
TEST(DebugApiServer, Healthz_Returns200) {
    ...
    EXPECT_EQ(result->status, 200) << "期望 HTTP 200, 实际: " << result->status;

    const std::string& body = result->body;
    EXPECT_TRUE(json_bool_true(body, "ok")) << "响应体缺 ok:true. body=" << body;
    EXPECT_TRUE(json_int64_positive(body, "as_of_ts")) << "响应体缺 as_of_ts > 0. body=" << body;
...
TEST(DebugApiServer, Version_Fields) {
    ...
    const std::string& body = result->body;
    EXPECT_TRUE(json_str_field_exists(body, "version")) << "响应体缺 version 字段. body=" << body;
    EXPECT_TRUE(json_str_in(body, "build_mode", {"paper", "live", "backtest"}))
        << "build_mode 不在 {paper,live,backtest}. body=" << body;
    EXPECT_TRUE(json_str_field_exists(body, "git_hash")) << "响应体缺 git_hash 字段. body=" << body;
    EXPECT_TRUE(json_int64_positive(body, "as_of_ts")) << "响应体缺 as_of_ts > 0. body=" << body;
...
TEST(DebugApiServer, HotpathIsolation_ServerStop) {
    ...
    EXPECT_GE(elapsed, 90) << "主线程 sleep_for(100ms) 提前返回? elapsed=" << elapsed << "ms";
    EXPECT_LT(elapsed, 500) << "主线程疑似 hang, elapsed=" << elapsed << "ms";
...
TEST(DebugApiServer, Concurrent100_NoCrashNoHang) {
    ...
    EXPECT_EQ(succ + fail, k_concurrent) << "并发请求总数不对: succ=" << succ << " fail=" << fail;

    // 允许少量连接失败 (OS 端口/fd 限制), 但 server 不能 crash
    // 期望成功率 ≥ 90%
    EXPECT_GE(succ, k_concurrent * 9 / 10) << "并发成功率 < 90%: succ=" << succ << " / " << k_concurrent;

    // server 仍在运行 (未 crash)
    EXPECT_TRUE(srv.is_running()) << "100 并发后 server is_running() = false (可能 crash 或意外退出)";
