...
class CountingAuditEmitter final : public risk::AuditEmitter {
 public:
    ...
    [[nodiscard]] bool emit(risk::AuditRecord const& rec) noexcept override {
        ++emitted_total_;
        if (rec.decision == risk::Decision::APPROVED)      ++emitted_approved_;
        else if (rec.decision == risk::Decision::REJECTED) ++emitted_rejected_;
        else                                               ++emitted_deferred_;
        const auto idx = static_cast<std::size_t>(rec.reject);
        if (idx < reject_hist_.size()) reject_hist_[idx] += 1;

        if (paper_audit_writer_ == nullptr) return true;
        ...
        const std::int64_t now_ns =
            stcpp::infra::wal::pit::NowRealtimeNs();
        const bool source_pit_ok =
            (rec.event_ts_ns       >  0)
            && (rec.data_source_ts_ns >= rec.event_ts_ns)
            && (rec.ingestion_ts_ns   >= rec.data_source_ts_ns)
            && (rec.as_of_ts_ns       >= rec.ingestion_ts_ns)
            && (rec.as_of_ts_ns       <= now_ns);
        if (!source_pit_ok) {
...
class PaperE2EFixture : public ::testing::Test {
 protected:
    void SetUp() override {
        ...
        auto open = [](WalKind k, const char* suffix)
            -> std::unique_ptr<WalWriter<AuditRecord>> {
            WalConfig cfg{};
            cfg.kind        = k;
        ...
        signer_  = std::make_unique<signer::paper::PaperSigner>(nonce_.get(), gas_.get(),
                                                                confirm_.get());
    ...
    [[nodiscard]] risk::OrderIntent MakeValidIntent(const PmBookUpdate& book,
    ...
    [[nodiscard]] E2EOutcome RunOneE2E(const PmBookUpdate& book,
                                       const std::string& signal_id) {
    ...
    std::unique_ptr<stcpp::infra::wal::WalWriter<stcpp::observability::AuditRecord>>
        paper_audit_, risk_audit_, position_, shadow_audit_;
