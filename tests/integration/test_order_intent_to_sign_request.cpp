...
risk::OrderIntent MakeValidIntent(
    ...
    intent.price               = 0.55;
    intent.size_usdc           = 100'00;  // $100 整 cent
...
std::array<std::uint8_t, 16> MakeNonZeroAuditId() noexcept {
    ...
    // 模拟 ULID 随机段 (非全零)
    id[0]  = 0x01U; id[1]  = 0xA2U; id[2]  = 0xB3U; id[3]  = 0xC4U;
    id[4]  = 0xD5U; id[5]  = 0xE6U; id[6]  = 0xF7U; id[7]  = 0x08U;
    id[8]  = 0x19U; id[9]  = 0x2AU; id[10] = 0x3BU; id[11] = 0x4CU;
    id[12] = 0x5DU; id[13] = 0x6EU; id[14] = 0x7FU; id[15] = 0x80U;
    return id;
}
...
TEST(OrderIntentToSignRequest, T1_YesBuy_SignAndVerify) {
    ...
    const auto intent = MakeValidIntent(
        ...
        strategy::Side::Buy, strategy::Outcome::Yes);
    ...
    EXPECT_NE(req.market_id, token_id)
        << "T1 spec-4: market_id must NOT be token_id (rename spec)";

    // spec-1: token_id 原样传入签名器
    EXPECT_EQ(req.token_id, token_id)
        << "T1 spec-1: token_id must pass through unchanged";

    // spec-2: side=Buy → 0
    EXPECT_EQ(req.side, 0U)
        << "T1 spec-2: Buy side must be 0";

    // spec-8: signature_type 必须 = 1 (默认值, 不触碰)
    EXPECT_EQ(req.signature_type, 1U)
        << "T1 spec-8: signature_type must be 1 (HMAC bug #2 防御)";

    // spec-6: audit_id echo 正确
    EXPECT_EQ(req.audit_id, audit_id)
        << "T1 spec-6: audit_id must come from caller (RiskDecision.audit_id)";
    ...
    const bool verify_ok = crypto::Ed25519::verify(
    ...
    EXPECT_TRUE(verify_ok)
        << "T1: Ed25519::verify of to_sign_request + Sign must return true "
...
TEST(OrderIntentToSignRequest, T2_NoSell_SideIsOne_ErrorIsOk) {
    ...
    const auto intent = MakeValidIntent(
        condition_id, token_id,
        strategy::Side::Sell, strategy::Outcome::No);
    ...
    // 禁布尔运算 / 条件重映射
    ASSERT_EQ(req.side, 1U)
        << "T2 spec-2: Sell side must be exactly 1 (static_cast<uint8_t>(Side::Sell))";
    ...
    EXPECT_EQ(req.market_id, condition_id)
        << "T2 spec-4: market_id must be condition_id";

    // spec-1: token_id 零变换
    EXPECT_EQ(req.token_id, token_id)
        << "T2 spec-1: token_id must pass through unchanged";

    // spec-8: signature_type = 1 (默认, 不触碰)
    EXPECT_EQ(req.signature_type, 1U)
        << "T2 spec-8: signature_type must be 1 (HMAC bug #2 防御, 不触碰)";

    // spec-6: audit_id echo
    EXPECT_EQ(req.audit_id, audit_id)
        << "T2 spec-6: audit_id must come from caller input";

    // --- Sign 必须 Ok (老沈 spec T2 硬要求: response.error == Ok) ---
    v52::SignerV52 signer{execution::ExecutionMode::Paper};
    const auto resp = signer.Sign(req);

    EXPECT_EQ(resp.error, v52::SignV52Error::Ok)
        << "T2: Sign must return Ok for valid NO Sell intent";
    EXPECT_EQ(resp.signature.size(), 64U)
        << "T2: signature must be 64B when Ok";
}
...
TEST(OrderIntentToSignRequest, T3_TokenId_ByteEqual) {
    const std::string condition_id =
        "0xdeadbeefcafe1234deadbeefcafe1234deadbeefcafe1234deadbeefcafe1234";
    const auto audit_id = MakeNonZeroAuditId();
