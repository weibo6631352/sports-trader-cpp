...
TEST_F(CRest03Fixture, C_REST_03_rest_timeout_no_hang_wss_unblocked) {
    ...
    EXPECT_EQ(clob_delay_ms_, 10'000) << "C-REST-03: REST 超时已注入 (delay=10s)";
    ...
    if (!fault_state_.wss_tick_latencies_ns.empty()) {
        ...
        EXPECT_LT(p99_us, 50LL) << "C-REST-03 / R-12: wss tick p99 " << p99_us << "us 必须 < 50us";
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        EXPECT_NO_FATAL_FAILURE({
            ...
            (void)out;
        }) << "C-REST-03: REST 超时期间 RM 不 hang / abort i="
           << i;
    }
}
