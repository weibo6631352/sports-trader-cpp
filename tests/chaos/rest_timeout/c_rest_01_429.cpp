...
TEST_F(CRest01Fixture, C_REST_01_429_rate_limit_backoff_no_halt) {
    ...
    EXPECT_EQ(fault_state_.rest_error_count, 1u)
        << "C-REST-01: REST error count 记录 1 (429 注入)";
    ...
    if (!fault_state_.wss_tick_latencies_ns.empty()) {
        ...
        EXPECT_LT(p99_us, 50LL)
            << "C-REST-01 / R-12: wss tick p99 " << p99_us << "us 必须 < 50us";
    }
    ...
    EXPECT_EQ(halt_count, 0u)
        << "C-REST-01: 429 不误判为 server down (STATE_HALTED reject == 0)";
}
