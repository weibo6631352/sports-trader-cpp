...
TEST_F(COoo01Fixture, C_OOO_01_seq_gap_triggers_rest_fallback) {
    ...
    EXPECT_TRUE(fault_state_.seq_gap_detected)
        << "C-OOO-01: seq gap=1 → seq_gap_detected must be true";
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        EXPECT_GE(b.data_source_ts_ns, b.event_ts_ns)
                                              << "C-OOO-01: data_source_ts >= event_ts (R-20)";
