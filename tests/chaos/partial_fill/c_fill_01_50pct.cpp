...
TEST_F(CFill01Fixture, C_FILL_01_50pct_partial_fill_r11_paper_audit) {
    ...
    EXPECT_DOUBLE_EQ(partial_fill_ratio_, 0.5)
        << "C-FILL-01: fill_ratio=0.5 注入";
    EXPECT_EQ(partial_fill_count_, 2)
        << "C-FILL-01: fill_count=2 注入";
    ...
    EXPECT_EQ(position_->HighWatermark(), 0u)
        << "C-FILL-01 / R-11: paper mode 不写 position WAL";
}
