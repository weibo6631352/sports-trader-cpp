...
TEST_F(CFill02Fixture, C_FILL_02_5pct_many_fills_no_exceed_exposure) {
    ...
    EXPECT_DOUBLE_EQ(partial_fill_ratio_, 0.05) << "C-FILL-02: fill_ratio=0.05 注入";
    EXPECT_EQ(partial_fill_count_, 20) << "C-FILL-02: fill_count=20 注入";
    ...
    EXPECT_EQ(sign_through, fill_wal_ok) << "C-FILL-02 / R-11: 全部 fill.audit_wal_kind == PaperAudit";
    ...
    EXPECT_EQ(exceed_count, 0u) << "C-FILL-02: 不触发 EXCEED_MARKET_EXPOSURE (小单不超额)";

    // R-11: position WAL 不写入
    EXPECT_EQ(position_->HighWatermark(), 0u) << "C-FILL-02 / R-11: paper mode 不写 position WAL";
}
