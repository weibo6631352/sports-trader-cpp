...
TEST_F(CFill04PaperFixture, C_FILL_04_paper_wal_partial_fill_r11) {
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (out.went_through_signer) {
            ...
            if (out.sign_resp.audit_wal_kind == WalKind::PaperAudit) ++sign_wal_ok;
        }
    }
    ...
    EXPECT_EQ(sign_through, fill_wal_ok)
        << "C-FILL-04 / R-11: 全部 VirtualFill.audit_wal_kind == PaperAudit";
    ...
    EXPECT_EQ(risk_audit_->HighWatermark(), 0u)
        << "C-FILL-04 / R-11: live risk_audit WAL 未被写入";
    EXPECT_EQ(position_->HighWatermark(), 0u)
        << "C-FILL-04 / R-11: live position WAL 未被写入";
    EXPECT_EQ(shadow_audit_->HighWatermark(), 0u)
        << "C-FILL-04 / R-11: shadow_audit WAL 未被写入";
}
