...
TEST_F(CPaper02Fixture, C_PAPER_02_ooo_5intents_r20_monotone_no_stale) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (out.rm_decision.is_approved()) ++approved_count;
    }

    // C-PAPER-02 核心断言
    EXPECT_EQ(r20_violations, 0)
        << "C-PAPER-02: 5 笔 intent 全部满足 R-20 4 ts 单调 (0 违例)";
    ...
    EXPECT_EQ(position_->HighWatermark(), 0u)
        << "C-PAPER-02: paper mode 不写 position WAL (R-11)";
}
