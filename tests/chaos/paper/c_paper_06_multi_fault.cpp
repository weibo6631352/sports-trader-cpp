...
TEST_F(CPaper06Fixture, C_PAPER_06_multi_fault_no_crash_audit_chain_intact) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        EXPECT_NO_FATAL_FAILURE({
            ...
            (void)out;
        }) << "C-PAPER-06: WSS断+REST慢组合故障下 RM 不崩 i="
           << i;
    }
    ...
    EXPECT_GT(audit_emitter_->emitted_total(), 0u) << "C-PAPER-06: audit chain 不断 (emitter 持续工作)";

    // R-11: paper mode 不写 position WAL
    EXPECT_EQ(position_->HighWatermark(), 0u) << "C-PAPER-06: paper mode 不写 position WAL (R-11)";
}
